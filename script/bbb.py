...
class BigBlueButtonExtractor:
    # BigBlueButton playback URLs come in three path variants:
    #   v1) /playback/presentation/<ver>/playback.html?meetingId=<id>            (_VALID_URL)
    #   v2) /playback/presentation/<ver>/<id>                                     (_VALID_URL2)
    #   v3) /playback/presentation/<ver>/<hex-id>-<timestamp>                    (_VALID_URL3, strictest of v2)
    # Storage endpoints are namespaced by the full record id (incl. any -<ts>
    # suffix), e.g. /presentation/<recordId>/video/webcams.webm — so the capture
    # group <id> keeps whatever the path segment holds.
    _VALID_URL  = r'(?P<website>https?://[^/]+)/playback/presentation/(?P<version>[0-9.]+)/playback\.html\?meetingId=(?P<id>[0-9a-fA-F\-]+)'
    _VALID_URL2 = r'(?P<website>https?://[^/]+)/playback/presentation/(?P<version>[0-9.]+)/(?P<id>[0-9a-fA-F\-]+).*'
    _VALID_URL3 = r'(?P<website>https?://[^/]+)/playback/presentation/(?P<version>[0-9.]+)/(?P<id>[0-9a-fA-F]+\-[0-9]+)'
    ...
    def _real_extract(self, url):
        ...
        if m is None:
            self._VALID_URL_RE = re.compile(self._VALID_URL3)
            m = self._VALID_URL_RE.match(url)
        if m is None:
